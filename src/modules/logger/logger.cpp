/* rtframe: replaced PX4-specific includes */
/* console_buffer.h depends on px4_platform_common — stub out instead */
static inline int px4_console_buffer_size() { return 0; }
static inline int px4_console_buffer_read(char *, int, int *) { return 0; }
...
/* rtframe: version stubs */
static inline const char *px4_firmware_version_string() { return "rtframe-1.0"; }
static inline uint32_t px4_firmware_version() { return 0x01000000; }
static inline uint32_t px4_firmware_vendor_version() { return 0; }
static inline const char *px4_board_name() { return "NXP-RT1170-CM7"; }
static inline const char *px4_board_sub_type() { return ""; }
static inline const char *px4_os_name() { return "Zephyr"; }
static inline const char *px4_os_version_string() { return KERNEL_VERSION_STRING; }
static inline uint32_t px4_os_version() { return KERNEL_VERSION_NUMBER; }
static inline const char *px4_firmware_git_branch() { return ""; }
static inline const char *px4_firmware_oem_version_string() { return ""; }
static inline const char *px4_toolchain_name() { return "GCC"; }
static inline const char *px4_toolchain_version() { return __VERSION__; }
static inline int board_mcu_version(char *rev, const char **chip, const char **chip_desc) { return -1; }

/* rtframe: cpuload stubs */
static inline void cpuload_monitor_stop() {}
static inline void init_print_load(print_load_s *) {}
static inline void print_load_buffer(char *, int, void(*)(void*), void *, print_load_s *) {}

/* rtframe: lockstep stubs */
static inline int px4_lockstep_register_component() { return -1; }
static inline void px4_lockstep_unregister_component(int) {}
static inline void px4_lockstep_progress(int) {}

/* rtframe: shutdown stub */
static inline int px4_register_shutdown_hook(bool(*)()) { return 0; }
static inline int px4_unregister_shutdown_hook(bool(*)()) { return 0; }

/* rtframe: px4_getopt stub — delegate to standard getopt */
static inline int px4_getopt(int argc, char *argv[], const char *opts,
                              int *optind_out, const char **optarg_out)
{
    extern char *optarg;
    extern int optind;
    *optind_out = optind;
...
static inline void send(uint32_t, int, const char *, Args...) {}
static inline int internalLogLevel(uint8_t levels) { return (levels >> 4) & 0xF; }
static constexpr int LogLevelInternal_Disabled = 8;
struct LogLevelInternal { static constexpr int Disabled = 8; static constexpr int Warning = 3; };
static inline uint32_t ID(const char *) { return 0; }
enum Log { Info = 6 };
}

/* rtframe: mavlink_log stub */
static inline void mavlink_log_info(orb_advert_t *, const char *, ...) {}

/* rtframe: checksums stub */
namespace component_information {
static constexpr const char *all_events_sha256 = "";
}

/* rtframe: replay stub */
namespace px4 { namespace replay {
static constexpr const char *ENV_FILENAME = "PX4_SIM_MODEL";
}}

/* rtframe: UUID stub */
#define BOARD_HAS_NO_UUID
#define PX4_GUID_FORMAT_SIZE  33
static inline void board_get_px4_guid_formated(char *buf, size_t len) { buf[0] = '\0'; }
...
/* rtframe: Zephyr k_timer expiry callback */
static void timer_callback(struct k_timer *timer)
{
	Logger::timer_callback_data_s *data =
		(Logger::timer_callback_data_s *)k_timer_user_data_get(timer);

	/* count = number of pending wakeups */
	int semaphore_value = k_sem_count_get(&data->semaphore);
...
bool Logger::initialize_topics()
{
	...
	if ((sdlog_profile & SDLogProfileMask::RAW_IMU_ACCEL_FIFO) || (sdlog_profile & SDLogProfileMask::RAW_IMU_GYRO_FIFO)) {
		...
		_log_interval = 800;
		/* rtframe: skip pthread priority change — Zephyr thread priority set at thread creation */
	}
...
void Logger::run()
{
	...
	px4_register_shutdown_hook(nullptr);
	...
	/* init the update timer — rtframe: use Zephyr k_sem + k_timer */
	k_sem_init(&_timer_callback_data.semaphore, 0, K_SEM_MAX_LIMIT);
	...
	if (_polling_topic_meta) {
		...
		/* rtframe: watchdog_initialize stubbed out (no NuttX task-monitor) */
		k_timer_init(&_loop_timer, timer_callback, nullptr);
		k_timer_user_data_set(&_loop_timer, &_timer_callback_data);
		k_timer_start(&_loop_timer,
			      K_USEC(_log_interval), K_USEC(_log_interval));
	}
	...
	while (!should_exit()) {
		...
		if (polling_topic_sub >= 0) {
			...
			/* rtframe: use standard POSIX poll for topic fd */
			struct pollfd fds[1];
			fds[0].fd = polling_topic_sub;
			fds[0].events = POLLIN;
			int pret = poll(fds, 1, 20);
			...
			if (pret < 0) {
				...
				if (fds[0].revents & POLLIN) {
					// need to do an orb_copy so that the next poll will not return immediately
					orb_copy(_polling_topic_meta, polling_topic_sub, _msg_buffer);
				}
			}

		} else {
			/* rtframe: wait on Zephyr semaphore, posted by k_timer callback */
			k_sem_take(&_timer_callback_data.semaphore, K_FOREVER);
		}
	}
	...
	/* rtframe: stop k_timer and destroy semaphore */
	k_timer_stop(&_loop_timer);
	k_sem_reset(&_timer_callback_data.semaphore);
	...
	px4_unregister_shutdown_hook(nullptr);
}
